// js/dartboard.js
// Builds an interactive, clickable SVG dartboard with segments for S/T/D and bulls.
// Calls window.pickThrow(name, value, isDouble) when a region is clicked.

(function(){
  const ORDER = [20,1,18,4,13,6,10,15,2,17,3,19,7,16,8,11,14,9,12,5];
  const ns = "http://www.w3.org/2000/svg";

  // Container where SVG will be injected (existing element in index.html)
  const container = document.querySelector('.board-wrap') || document.getElementById('dartboardContainer') || document.body;
  // Create SVG
  const SVG_SIZE = 720; // high-res, scales down using CSS
  const cx = SVG_SIZE/2, cy = SVG_SIZE/2;
  const svg = document.createElementNS(ns,'svg');
  svg.setAttribute('id','interactive-dartboard');
  svg.setAttribute('viewBox',`0 0 ${SVG_SIZE} ${SVG_SIZE}`);
  svg.setAttribute('width','100%');
  svg.setAttribute('height','100%');
  svg.setAttribute('role','img');
  svg.setAttribute('aria-label','Interactive dartboard');

  // Radii (tweak visually if needed)
  const R_OUTER = SVG_SIZE*0.48;         // outermost edge
  const R_DOUBLE_IN = R_OUTER * 0.88;
  const R_DOUBLE_OUT = R_OUTER;
  const R_SINGLE_OUTER_IN = R_DOUBLE_IN;
  const R_SINGLE_OUTER_OUT = R_OUTER * 0.72;
  const R_TRIPLE_IN = R_OUTER * 0.56;
  const R_TRIPLE_OUT = R_OUTER * 0.64;
  const R_SINGLE_INNER_IN = R_TRIPLE_IN;
  const R_SINGLE_INNER_OUT = R_TRIPLE_OUT;
  const R_BULL_OUTER = R_OUTER * 0.12;  // outer bull radius
  const R_BULL_INNER = R_OUTER * 0.05;  // inner bull radius

  // helper: polar to cartesian
  function pt(angleDeg, radius){
    const a = (angleDeg - 90) * Math.PI/180;
    return { x: cx + Math.cos(a)*radius, y: cy + Math.sin(a)*radius };
  }

  function makeWedgePath(rIn, rOut, angA, angB){
    // Build arc path from angA -> angB (degrees), outer arc then inner arc back
    const p1 = pt(angA, rOut);
    const p2 = pt(angB, rOut);
    const p3 = pt(angB, rIn);
    const p4 = pt(angA, rIn);
    const large = (angB - angA) > 180 ? 1 : 0;
    // A rx ry x-axis-rotation large-arc-flag sweep-flag x y
    return `M ${p1.x} ${p1.y} A ${rOut} ${rOut} 0 ${large} 1 ${p2.x} ${p2.y} L ${p3.x} ${p3.y} A ${rIn} ${rIn} 0 ${large} 0 ${p4.x} ${p4.y} Z`;
  }

  function createPath(d, attrs){
    const p = document.createElementNS(ns,'path');
    p.setAttribute('d', d);
    for(const k in attrs) p.setAttribute(k, attrs[k]);
    return p;
  }

  // Build board group
  const g = document.createElementNS(ns,'g');
  g.setAttribute('transform', `translate(0,0)`);
  svg.appendChild(g);

  // draw background circle
  const bg = document.createElementNS(ns,'circle');
  bg.setAttribute('cx', cx);
  bg.setAttribute('cy', cy);
  bg.setAttribute('r', R_OUTER);
  bg.setAttribute('fill', '#0b0b0b');
  g.appendChild(bg);

  // Draw segments: for each wedge, create double, single outer, triple, single inner
  const wedgeAngle = 360 / 20;
  for(let i=0;i<20;i++){
    const angA = i * wedgeAngle;
    const angB = (i+1) * wedgeAngle;
    const num = ORDER[i];

    // DOUBLE ring (outer)
    const dDouble = makeWedgePath(R_DOUBLE_IN, R_DOUBLE_OUT, angA, angB);
    const colorDouble = (i % 2 === 0) ? '#d62e2e' : '#0b8a3d';
    const elD = createPath(dDouble, {
      fill: colorDouble,
      stroke: '#07090b',
      'stroke-width': 0.8,
      'data-name': 'D' + num,
      'data-value': String(2 * num),
      'data-isdouble': '1',
      class: 'db-seg'
    });
    g.appendChild(elD);

    // Single outer
    const dSout = makeWedgePath(R_SINGLE_OUTER_IN, R_SINGLE_OUTER_OUT, angA, angB);
    const colorSout = (i % 2 === 0) ? '#f2f2f2' : '#0f1111';
    const elSO = createPath(dSout, {
      fill: colorSout,
      stroke: '#07090b',
      'stroke-width': 0.6,
      'data-name': 'S' + num,
      'data-value': String(num),
      'data-isdouble': '0',
      class: 'sout-seg'
    });
    g.appendChild(elSO);

    // Triple ring
    const dTriple = makeWedgePath(R_TRIPLE_IN, R_TRIPLE_OUT, angA, angB);
    const colorTriple = (i % 2 === 0) ? '#d62e2e' : '#0b8a3d';
    const elT = createPath(dTriple, {
      fill: colorTriple,
      stroke: '#07090b',
      'stroke-width': 0.6,
      'data-name': 'T' + num,
      'data-value': String(3 * num),
      'data-isdouble': '0',
      class: 't-seg'
    });
    g.appendChild(elT);

    // Single inner
    const dSin = makeWedgePath(R_SINGLE_INNER_IN, R_SINGLE_INNER_OUT, angA, angB);
    const colorSin = (i % 2 === 0) ? '#f2f2f2' : '#0f1111';
    const elSI = createPath(dSin, {
      fill: colorSin,
      stroke: '#07090b',
      'stroke-width': 0.6,
      'data-name': 'S' + num,
      'data-value': String(num),
      'data-isdouble': '0',
      class: 'sin-seg'
    });
    g.appendChild(elSI);

    // (Optional) Add thin separators or number labels later
  }

  // Bull rings: outer bull (SB) and inner bull (DB)
  const sb = document.createElementNS(ns,'circle');
  sb.setAttribute('cx', cx);
  sb.setAttribute('cy', cy);
  sb.setAttribute('r', R_BULL_OUTER);
  sb.setAttribute('fill','#0b8a3d');
  sb.setAttribute('data-name','SB');
  sb.setAttribute('data-value','25');
  sb.setAttribute('data-isdouble','0');
  sb.setAttribute('class','sbull');
  g.appendChild(sb);

  const db = document.createElementNS(ns,'circle');
  db.setAttribute('cx', cx);
  db.setAttribute('cy', cy);
  db.setAttribute('r', R_BULL_INNER);
  db.setAttribute('fill','#d62e2e');
  db.setAttribute('data-name','DB');
  db.setAttribute('data-value','50');
  db.setAttribute('data-isdouble','1');
  db.setAttribute('class','dbull');
  g.appendChild(db);

  // Numbers ring (draw small text labels around board)
  const numGroup = document.createElementNS(ns,'g');
  numGroup.setAttribute('class','numbers');
  g.appendChild(numGroup);
  const labelRadius = R_OUTER * 1.02;
  for(let i=0;i<20;i++){
    const ang = (i + 0.5) * wedgeAngle; // center of wedge
    const pos = pt(ang, labelRadius);
    const text = document.createElementNS(ns,'text');
    text.setAttribute('x', pos.x);
    text.setAttribute('y', pos.y);
    text.setAttribute('text-anchor','middle');
    text.setAttribute('dominant-baseline','central');
    text.setAttribute('font-size', String(Math.round(SVG_SIZE*0.045)));
    text.setAttribute('fill','#ffffff');
    text.setAttribute('pointer-events','none');
    text.textContent = ORDER[i];
    numGroup.appendChild(text);
  }

  // attach event listeners (delegation) for click + hover
  svg.addEventListener('click', function(evt){
    const target = evt.target;
    // find nearest element with data-name
    const el = target.closest && target.closest('[data-name]') || (target.getAttribute && target.getAttribute('data-name') ? target : null);
    if(!el) return;
    const name = el.getAttribute('data-name');
    const value = parseInt(el.getAttribute('data-value') || '0', 10) || 0;
    const isDouble = el.getAttribute('data-isdouble') === '1';
    // add visual feedback: flash/pulse the clicked element
    el.classList.add('highlight');
    setTimeout(()=>el.classList.remove('highlight'), 700);
    // call host app function if present
    if(typeof window.pickThrow === 'function'){
      try{ window.pickThrow(name, value, isDouble); }catch(e){ console.warn('pickThrow failed', e); }
    } else {
      // fallback: log
      console.log('clicked', name, value, isDouble);
    }
  }, {passive:true});

  // mouseover visual (optional)
  svg.addEventListener('pointerenter', ()=>{}, true);

  // place svg into container: prefer a dedicated container element
  // If there's an element with id "board-svg-root" or "dartboardContainer" use it
  const rootInsert = document.getElementById('board-svg-root') || document.getElementById('dartboardContainer') || document.querySelector('.board-wrap') || container;
  // if container is the .board-wrap card, insert before chips area if exists
  if(rootInsert === container && container.children.length) container.insertBefore(svg, container.firstElementChild);
  else rootInsert.appendChild(svg);

  // expose helper to redraw (not strictly needed)
  window.rebuildInteractiveBoard = function(){ /* placeholder in case we want to rebuild */ };

})();
